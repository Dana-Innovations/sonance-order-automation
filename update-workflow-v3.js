...
return { json: lineItem };`
    ...
    console.log(`\nStep 5: Preparing update payload...`);

    // Only send the fields that are allowed in PUT request
    const updatePayload = {
      name: workflow.name,
    ...
    if (workflow.description) updatePayload.description = workflow.description;
    if (workflow.active !== undefined) updatePayload.active = workflow.active;

    console.log(`  Nodes: ${updatePayload.nodes.length}`);
    console.log(`  Connections: ${Object.keys(updatePayload.connections).length}`);

    console.log('\nStep 6: Sending updated workflow...');
    const result = await makeRequest('PUT', `/api/v1/workflows/${WORKFLOW_ID}`, updatePayload);

    console.log('\n✅ SUCCESS! Workflow updated.');
    console.log(`   Total nodes: ${result.nodes ? result.nodes.length : updatePayload.nodes.length}`);
    ...
    console.error('\n❌ Error:', error.message);
    console.error('\nIf backup exists, you can restore with the backup file.');
  }
}
