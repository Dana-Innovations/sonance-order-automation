...
export function WizardStep15({ session, onNext, isLoading }: WizardStepProps) {
  const [generating, setGenerating] = useState(false)
  const [generatedPrompt, setGeneratedPrompt] = useState(
    session.customer_data?.order_line_prompt || ''
  )
  const [isEditing, setIsEditing] = useState(false)

  const lineAnswers = session.question_answers?.filter(qa => qa.wizard_type === 'order_line') || []
  const answeredCount = lineAnswers.filter(qa => qa.text_answer).length
  ...
  const handleSaveAndContinue = async () => {
    ...
    await onNext({
      order_line_prompt: generatedPrompt
    })
  }
    ...
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      <div className="mb-8">
        <h2 className="text-2xl font-semibold text-[#333F48] mb-2 flex items-center gap-2">
          ...
          Generate Order Line Item Prompt
        </h2>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
        ...
        <ul className="text-sm text-[#6b7a85] space-y-1">
          <li>✓ {answeredCount} line item questions answered</li>
      ...
      {!generatedPrompt && !generating && (
        <div className="text-center py-12">
          <p className="text-[#6b7a85] mb-6">
            Click the button below to use AI to generate a custom line item extraction prompt
      ...
      {generatedPrompt && !generating && (
        <div className="mb-6">
          ...
          <p className="text-xs text-[#6b7a85] mt-2">
            ...
            to extract line item data from PDFs.
          </p>
        </div>
      )}
