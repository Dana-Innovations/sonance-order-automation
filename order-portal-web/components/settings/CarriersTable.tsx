        ...
        <tbody className="divide-y divide-[#D9D9D6]">
          {carriers.length === 0 ? (
            ...
            carriers.map((carrier) => (
              <tr key={`${carrier.carrier_id}-${carrier.ship_via_code}`} className="bg-white hover:bg-[#F5F5F5]/50 transition-colors">
                ...
                <td className="px-4 text-[#333F48] font-medium" style={{ fontSize: '15px', paddingTop: '10px', paddingBottom: '10px' }}>
                  {carrier.ship_via_code}
                </td>
                <td className="px-4 text-[#333F48]" style={{ fontSize: '15px', paddingTop: '10px', paddingBottom: '10px' }}>
                  {carrier.ship_via_desc}
                </td>
                <td className="px-4" style={{ fontSize: '15px', paddingTop: '10px', paddingBottom: '10px', color: carrier.is_active ? '#333F48' : '#DC2626' }}>
                  {carrier.is_active ? 'Active' : 'Inactive'}
                </td>
                <td className="px-4 text-[#6b7a85]" style={{ fontSize: '14px', paddingTop: '10px', paddingBottom: '10px' }}>
                  {carrier.updated_at ? format(new Date(carrier.updated_at), 'MMM d, yyyy') : 'N/A'}
                </td>
                <td className="px-4 text-right" style={{ paddingTop: '10px', paddingBottom: '10px' }}>
                  <Link
                    href={`/settings/carriers/${carrier.carrier_id}/${carrier.ship_via_code}/edit`}
